...
last_drawed_number = None
last_box_number = None

already_won = {}

for drawed_number in numbers_to_find:
    print("Number drawed => " + str(drawed_number))
        ...
        for j in range(len(grid[i])):
            if grid[i][j] == drawed_number:
                ...
                if  marker_grid[i][0] == 1 and marker_grid[i][1] == 1 and marker_grid[i][2] == 1 and marker_grid[i][3] == 1 and marker_grid[i][4] == 1:
                    if row_index not in already_won:
                        already_won[row_index] = True
                        last_drawed_number = drawed_number
                        last_box_number = row_index

                    print(already_won)

                    game_done = True
                    for i in range(int(len(grid)/5)):
                        if (i*5) not in already_won:
                            game_done = False
                    ...
                    if game_done is True:
                        ...
                        for k in range(5):
                            for l in range(5):
                                if marker_grid[last_box_number + k][l] == 0:
                                    box_sum = box_sum + grid[last_box_number + k][l]


                        print(box_sum * last_drawed_number)
                        exit()
